...
typedef YacResult (*yapiFuncGetEnvAttr)(YacHandle hEnv, YapiEnvAttr attr, void* value, int32_t bufLength, int32_t* stringLength);

typedef YacResult (*yapiFuncConnect)(YacHandle hConn, const char* url, int16_t urlLength, const char* user, int16_t userLength,
...
typedef YacResult (*yapiFuncGetConnAttr)(YacHandle hConn, YapiConnAttr attr, void* value, int32_t bufLength, int32_t* stringLength);
typedef YacResult (*yapiFuncSetStmtAttr)(YacHandle hStmt, YapiStmtAttr attr, void* value, int32_t length);
typedef YacResult (*yapiFuncGetStmtAttr)(YacHandle hStmt, YapiStmtAttr attr, void* value, int32_t bufLength, int32_t* stringLength);
...
typedef YacResult (*yapiFuncBindColumn)(YacHandle hStmt, uint16_t id, YapiType type, void* value, int32_t bufLen,
                        int32_t* indicator);
typedef YacResult (*yapiFuncBindParameter)(YacHandle hStmt, uint16_t id, YapiParamDirection direction, YapiType bindType,
                           void* value, uint32_t bindSize, int32_t bufLength, int32_t* indicator);
typedef YacResult (*yapiFuncBindParameterByName)(YacHandle hStmt, char* name, YapiParamDirection direction, YapiType bindType,
                                 void* value, uint32_t bindSize, int32_t bufLength, int32_t* indicator);
...
typedef YacResult (*yapiFuncLobRead)(YacHandle hConn, YapiLobLocator* loc, uint64_t* bytes, uint8_t* buf, uint64_t bufLen);
typedef YacResult (*yapiFuncLobWrite)(YacHandle hConn, YapiLobLocator* loc, uint64_t* bytes, uint8_t* buf, uint64_t bufLen);
